...
# Intervention type: pyvene, head_out
# Models: llama3_8B_instruct, llama2_chat_13B
# Intervention strength: Baseline, alpha=15 k=48

model_names=("llama2_chat_7B")
seeds=(1 2 3)
# # First, running validate_2fold.py:
...
# Second, running pv_validate_2fold.py:
for model_name in "${model_names[@]}"; do
    for pair in "0 1" "15 48"; do
        alpha=$(echo $pair | cut -d' ' -f1)
        K=$(echo $pair | cut -d' ' -f2)
        ...
        sbatch --job-name=pv_validate_2fold_${model_prefix}${model_name}_a${alpha}_k${K} \
                --output=${log_path}/pv_${model_prefix}${model_name}_a${alpha}_k${K}_seed${seed}.out \
                --error=${log_path}/pv_${model_prefix}${model_name}_a${alpha}_k${K}_seed${seed}.err \
                run_job_pv_validate_2fold.sh --model_name $model_name --model_prefix "" --num_heads $K --alpha $alpha --instruction_prompt $instruction_prompt --judge_name $judge_name --info_name $info_name --seed $seed
            sleep 60
        done
    done
done